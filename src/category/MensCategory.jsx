...
<div className="bg-white px-4 md:px-6 py-4 shadow flex flex-col md:flex-row items-center justify-between gap-4">
  {/* Logo */}
  <h1 className="text-3xl md:text-4xl font-extrabold text-orange-500">Efyer</h1>
  ...
  <div className="flex flex-col sm:flex-row gap-2 w-full md:w-2/5">
    ...
    <div className="flex w-full">
      ...
      />
      <button className="bg-orange-500 text-white px-4 py-2 text-sm">🔍</button>
    </div>
  </div>
  ...
  <div className="flex items-center gap-4">
    ...
    >
      ...
      {/* Swiper */}
      <section className="py-14 px-4 bg-gradient-to-br from-gray-100 to-gray-200">
        <div className="text-center mb-10">
          <h2 className="text-4xl font-bold text-gray-800">Men's Top Picks</h2>
          <p className="text-orange-500 text-lg mt-2">Elevate your style game</p>
        </div>
            ...
            <SwiperSlide key={product.id}>
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent text-white p-6 flex flex-col justify-end">
                  ...
                  <p className="text-orange-400 font-bold text-lg mb-4">{product.price}</p>
                  <motion.button
                    onClick={() => handleAddToCart(product)}
                    whileHover={{ scale: 1.05 }}
                    className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 px-4 py-2 rounded-md text-sm font-medium transition"
                  >
      ...
      <section className="bg-white py-10 px-6">
        <h2 className="text-3xl font-bold text-center text-orange-500 mb-8">Men's Fashion</h2>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((product, index) => (
            ...
            >
              ...
              >
                <FaHeart className={`text-lg ${liked.includes(index) ? "fill-orange-500" : ""}`} />
              </button>
              <img
                src={product.image}
                alt={product.name}
                className="w-90 ml-2 h-90 object-cover rounded-t-lg"
              />
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {todos.map((todo, index) => (
              ...
              >
                ...
                <img src={todo.image} alt={todo.name} className="w-full h-64 object-cover rounded-t-lg" />
                <div className="p-4 space-y-2">
                  <h3 className="font-semibold text-lg">{todo.title}</h3>
                  <p>{todo.description}</p>
                  <p className="text-orange-600 font-bold">{todo.price}</p>
                  <button
